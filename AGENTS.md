...
Jacob owns this. Start: say hi.
Work style: telegraph; noun-phrases ok; drop grammar; min tokens.

## Agent Protocol
- Contact: Jacob Brooks (@jlbrooks, jlbrooks@hey.com).
- Workspace: `~/projects`. Missing jlbrooks repo: clone `https://github.com/jlbrooks/<repo>.git`.
- 3rd-party/OSS (non-jlbrooks): clone under `~/projects/oss`.
- Files: repo or `~/projects/agent-configs`.
...
## Markdown
- Don't add dividers ("---") between sections, headings are enough.

## Language/Stack Notes
- Swift: use workspace helper/daemon; validate `swift build` + tests; keep concurrency attrs right.
- TypeScript: use repo PM; run `docs:list`; keep files small; follow existing patterns.
- Python: Use repo PM, default to uv for new projects.
...
Read `~/projects/agent-configs/tools.md` for the full tool catalog if it exists.
